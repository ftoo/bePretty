...
class AfricasTalkingGateway
{
  ...
  //Messaging methods
  public function sendMessage($to_, $message_,$type_, $from_ = null, $bulkSMSMode_ = 1, Array $options_ = array())
  {
    if ( strlen($to_) == 0 || strlen($message_) == 0 ) {
      throw new AfricasTalkingGatewayException('Please supply both to and message parameters');
    }
    if($type_==1){
       $message ="Your Code is : ".$message_;
      }
    else if($type_==2){
    $message ="Your transaction is being processed.Your order number ".$message_;
     }else if($type_==3){
      $message =$message_;
     }
    else{
    $message ="Your Password is ".$message_;
     }
    $params = array(
